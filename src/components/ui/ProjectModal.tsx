    ...
    return createPortal(
        ...
        >
            ...
            >
                ...
                <div className="w-full md:w-2/3 h-[40vh] md:h-auto relative bg-black/50 group p-6 flex items-center justify-center">
                    ...
                    {images.length > 1 && (
                        <>
                            <button 
                                onClick={prevImage}
                                className="absolute left-2 top-1/2 -translate-y-1/2 bg-black/50 hover:bg-white/20 p-3 rounded-full text-white transition-all opacity-0 group-hover:opacity-100 z-10"
                            >
                                ←
                            </button>
                            <button 
                ...
                <div 
                    className="w-full md:w-1/3 flex flex-col bg-[#111] border-l border-white/5 relative"
                >
                    {/* Bouton Fermer (Absolu en haut à droite) */}
                    <button 
                        onClick={handleClose}
                        className="absolute top-6 right-6 z-20 w-10 h-10 flex items-center justify-center rounded-full bg-white/5 hover:bg-white/20 text-gray-400 hover:text-white transition-all backdrop-blur-sm"
                    >
                    ...
                    {/* Contenu Scrollable */}
                    <div 
                        className="flex-grow overflow-y-auto custom-scrollbar p-10 pt-16"
                        data-lenis-prevent
                    >
                        <div className="space-y-8">
                            ...
                            <div>
                                ...
                                <p className="text-gray-300 leading-relaxed font-light text-base md:text-lg whitespace-pre-line text-justify">
                                ...
                                <div>
                                    ...
                                    <ul className="space-y-3">
                                        {project.challenges.map((challenge, idx) => (
                                            <li key={idx} className="flex items-start gap-3 text-gray-300 font-light text-sm md:text-base">
                            ...
                            <div>
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {project.stack.map((tech) => (
                                        <span key={tech} className="px-4 py-2 border border-white/10 text-gray-400 text-xs rounded hover:border-white/40 hover:text-white transition-colors cursor-default bg-white/[0.02]">
                                            {tech}
                                        </span>
                                    ))}
                                </div>
                        ...
                        {/* Footer Buttons (Sticky bottom or just at the end) */}
